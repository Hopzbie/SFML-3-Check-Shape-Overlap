...
class Shape
{
    ...
    // function - check if shape overlaps with another shape
    bool CheckOverlap(Shape& shape)
    {
        // overlap from edges
        float R = (x + w) - shape.x;
        float L = (shape.x + shape.w) - x;
        float B = (y + h) - shape.y;
        float T = (shape.y + shape.h) - y;
        ...
        // set red channel of color
        if (R < L)  b = 128 + (1 - R/w)/2 * 255;
        else        b = 128 - (1 - L/w)/2 * 255;
        if (B < T)  g = 128 + (1 - B/h)/2 * 255;
        else        g = 128 - (1 - T/h)/2 * 255;
        r = 0;

        if (R > 0.f and L > 0.f and B > 0.f and T > 0.f)
            return true;
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        // check overlap
        if (shape_1.CheckOverlap(shape_2))
        {
